...
const row=(key:string,totalTokens:number,costUsd:number|null):Summary=>({key,label:key,totalTokens,costUsd,knownCostUsd:costUsd??0,unpricedTokens:costUsd==null?totalTokens:0,unpricedEvents:costUsd==null?1:0,events:1,lastTs:totalTokens,agent:'codex',session:key,path:'',tokens:{input:totalTokens,cached:0,cacheWrite:0,output:0,reasoning:0}});
