...
export function preset(minutes:number):Pick<Query,'start'|'end'|'offsetMinutes'> {const end=Math.floor(Date.now()/60000)*60000;return {start:end-minutes*60000,end,offsetMinutes:-new Date(end).getTimezoneOffset()};}
