...
export function readScanStatusRow(input: unknown): ScanStatusRow {
  ...
  const partial =
    ...
    unreadable > 0;
  return { ...(readCount(row.failedFiles) !== null ? {failedFiles: readCount(row.failedFiles)!, nextRetryAt: readCount(row.nextRetryAt)} : {}), agent, state, files, reused, malformedLines, errors, unreadableErrors: unreadable, updatedAt, partial };
}
