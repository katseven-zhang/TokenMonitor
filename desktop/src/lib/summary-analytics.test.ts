...
const row=(key:string,costUsd:number|null):Summary=>({key,label:key,agent:'codex',session:'',path:'',lastTs:1,events:1,totalTokens:100,costUsd,knownCostUsd:costUsd??0,unpricedTokens:costUsd==null?100:0,unpricedEvents:costUsd===null?1:0,tokens:{input:20,cached:50,cacheWrite:10,output:20,reasoning:5}});
