...
import { cacheHitRate,comparisonShares,comparisonTotals,effectivePrice } from '../lib/summary-analytics';
...
export function SummaryTable({rows,kind,onDrill}:{rows:Summary[];kind:SummaryKind;onDrill:(row:Summary,kind:SummaryKind)=>void}) {
  ...
  if(comparing) columns.push(['cacheHitRate','缓存命中率'],['effectivePrice','USD / 百万 Tokens']);
