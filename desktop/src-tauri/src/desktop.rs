...
fn dispatch(app: &tauri::AppHandle, method: &str, args: Value) -> Result<Value, String> {
    ...
    match method {
        "bootstrap" => Ok(
            json!({"settings":config::settings(&root)?,"prices":fs::read_to_string(root.join("prices.json")).map_err(|e|e.to_string())?,"dataDir":root,"agents":config::AGENTS,"autostart":autostart_enabled(app)?}),
        ),
        "status" => {
